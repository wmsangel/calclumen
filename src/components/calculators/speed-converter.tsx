...
const UNITS = ["mph", "km/h", "m/s", "knots", "ft/s"] as const;
type Unit = (typeof UNITS)[number];

/** Factors to metres per second: value_in_ms = value * factor. */
...
/** formatNumber(n, 3) with trailing zeros trimmed for a tidy result. */
function formatResult(n: number): string {
  const raw = formatNumber(n, 3);
  return raw.includes(".") ? raw.replace(/\.?0+$/, "") : raw;
}

export function SpeedConverter() {
  const [value, setValue] = useState("60");
  const [from, setFrom] = useState<Unit>("mph");
  const [to, setTo] = useState<Unit>("km/h");
